...
public class WYJobPageCrawler extends JobPageCrawler {
	...
	public void process(Page page) {
		if(page.getUrl().toString().contains("index")) {
			page.addTargetRequests(page.getHtml().links().all());
		}else {
			Map<String,Object> map = new HashMap<String,Object>();
			String id = page.getUrl().regex(pattern.pattern()).toString();
			map.put("id",id);
			map.put("resource",page.getUrl().toString());
			map.put("hdfs",job_rawdata_path);
			String jobConfig = ReadFile.ReadFile(System.getProperty("user.dir") + "/configuration/job_config.json");
			JSONObject jsonObject = new JSONObject(jobConfig);
			JSONArray jsonArray = jsonObject.getJSONArray("wbsites").getJSONObject(0).getJSONArray("fields");
			for(int i=0;i<jsonArray.length();i++) {
				JSONObject object = jsonArray.getJSONObject(i);
				String name = object.getString("name");
				String path = object.getString("path");
				if(path.charAt(0)=='/') {
					map.put(name,Format_transform.change(page.getHtml().xpath(path).toString()));
				}else {
					map.put(name,Format_transform.change(page.getHtml().regex(path).toString()));
				}
			}
			String[] cs = UtilTools.parseCompony(map.get("companymess").toString());
			if(cs.length<3)return;
			map.put("nature", cs[0]);
			map.put("scale", cs[1]);
			map.put("industry", cs[2]);
