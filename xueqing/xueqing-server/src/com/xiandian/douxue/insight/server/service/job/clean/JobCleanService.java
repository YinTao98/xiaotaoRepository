...
//		ServiceJob.submitCronJob(server, SERVICE_NAME, this.parent.getServiceName(), this, cron);//test 0/10 * * * * ?
	...
	public synchronized ServiceState process() {/////
		try {
			...
			jobDataReposity.cleanJobData();//清洗数据
			String jobConfig = ReadFile.ReadFile(System.getProperty("user.dir") + "/configuration/hbaseclassify.json");
			JSONObject jsonObject = new JSONObject(jobConfig);
			jobDataReposity.classify(jsonObject.getJSONArray("cloud"), "job_cloud", "cloud");
		} catch (Exception e) {
