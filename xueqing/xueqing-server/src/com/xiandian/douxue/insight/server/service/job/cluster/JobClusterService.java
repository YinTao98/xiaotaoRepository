...
public class JobClusterService implements Service {
	...
	public ServiceState process() {/////
		...
		String jobConfig = ReadFile.ReadFile(System.getProperty("user.dir") + "/configuration/job_industry_skills_config.json");
		JSONObject jsonObject = new JSONObject(jobConfig);
		JSONObject industry = jsonObject.getJSONArray("industry").getJSONObject(0);
		String[] categorys = industry.getString("category").split(",");
		List<String> dictionary = Arrays.asList(industry.getString("skills").split(","));
		for(String category : categorys)
			jobCluster.cluster("cloud", category, dictionary, 3, mongoClient);
