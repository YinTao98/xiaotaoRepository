...
//			jobname = jobDataReposity.queryDataByColumn("job_" + industry, industry, "JOB_NAME");
	...
	public List<Integer> kMeans(List<double[]> des1, int cluster) {
		...
		List<Vector> des = new ArrayList();
		for(double[] d : des1) {
			Vector v = Vectors.dense(d);
			des.add(v);
		}
		JavaRDD<Vector> jr = sc.parallelize(des);
		KMeansModel model = KMeans.train(jr.rdd(),cluster,100,3,KMeans.K_MEANS_PARALLEL());
		indexs = model.predict(jr).collect();
		sc.stop();
		return indexs;
	}
