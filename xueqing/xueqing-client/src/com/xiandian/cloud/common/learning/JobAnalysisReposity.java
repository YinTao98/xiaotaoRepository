...
public class JobAnalysisReposity {
	 ...
	 * @return
	 */
	private static ArrayList<String> mergeSkill(ArrayList<String> allSkills,String[] skills){
	 ...
	 * @return
	 */
	private static int[] getJobIds(ArrayList<String> allSkills,String[] skills) {
		int[] ids = new int[skills.length];
		for(int i=0;i<ids.length;i++) {
			ids[i]=allSkills.indexOf(skills[i].toLowerCase().trim());
		}
		return ids;
	}
	...
	public static Map<String, Object> getjytj(MongoClient mongoClient, String[] skills,String[] weights, int number) {
		...
		List<Map<String,Object>> jobMsg = new ArrayList<Map<String,Object>>();
		List<List<RaderObj>> raders = new ArrayList<List<RaderObj>>();
		List<Map<String,Object>> allJob = new ArrayList<Map<String,Object>>();
		ArrayList<String> allSkills = new ArrayList<String>();
		FastByIDMap<PreferenceArray> userData = new FastByIDMap<PreferenceArray>();
		mergeSkill(allSkills,skills);
		
		//读取MongoDB钟的employ的岗位数据
		MongoDatabase databases = mongoClient.getDatabase("employ");
		MongoCollection<Document> collection = databases.getCollection("job");
		FindIterable<Document> find = collection.find();
		MongoCursor<Document> cursor = find.iterator();
		while(cursor.hasNext()) {
			Document document = cursor.next();
			List<Document> jobs = (List<Document>)document.get("jobs");
			for(Document job : jobs) {
				int id = Integer.parseInt(job.getString("id"));
				String[] jobSkills = job.getString("skills").split(",");
				String[] jobWeights = job.getString("weight").split(",");
				mergeSkill(allSkills,jobSkills);
				int[] jobIds = getJobIds(allSkills,jobSkills);
				GenericPreference[] g = new GenericPreference[jobIds.length];
				for(int i=0;i<jobIds.length;i++) {
					g[i] = new GenericPreference(id,jobIds[i],Float.parseFloat(jobWeights[i]));
				}
				userData.put(id,new GenericUserPreferenceArray(Arrays.asList(g)));
				allJob.add(job);
			}
		}
		int[] userIds = getJobIds(allSkills,skills);
		GenericPreference[] g = new GenericPreference[userIds.length];
		for(int i=0;i<userIds.length;i++) {
			g[i] = new GenericPreference(0,userIds[i],Float.parseFloat(weights[i])/12);
		}
		userData.put(0,new GenericUserPreferenceArray(Arrays.asList(g)));
		...
		try {
			...
			//进行近邻计算
			NearestNUserNeighborhood neigh = new NearestNUserNeighborhood(number,userSimi,model);
			long[] ids = neigh.getUserNeighborhood(0);
			for(long id : ids) {
				for(Map<String,Object> job : allJob) {
					int jobId = Integer.parseInt(job.get("id").toString());
					if(jobId==id) {
						List<Double> vs = new ArrayList<Double>();
						List<RaderObj> rs = new ArrayList<RaderObj>();
						Map<String,Object> js = new HashMap<String,Object>();
						String[] jobSkills = job.get("skills").toString().split(",");
						String[] jobWeights = job.get("weight").toString().split(",");
						for(String s : jobSkills) {
							RaderObj r = new RaderObj();
							r.setName(s.toLowerCase().trim());
							r.setMax(1);
							rs.add(r);
						}
						for(int i=0;i<jobWeights.length;i++) {
							vs.add(Double.valueOf(jobWeights[i]));
						}
						js.put("jobname",job.get("job_name").toString());
						js.put("localtion",job.get("provice").toString()+"-"+job.get("city").toString());
						js.put("companyname",job.get("company_name").toString());
						values.add(vs);
						jobMsg.add(js);
						raders.add(rs);
						break;
